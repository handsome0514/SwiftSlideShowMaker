...
static const NSTimeInterval kMaxRecordingTime = 60.0;
...
- (NSURL *)recordedAudioFileURL {
    ...
    //return [NSURL fileURLWithPathComponents:pathComponents];
    NSString *filename = [Utilities generateRandomFileNameWithLength:48 fileExtension:@"m4a"];
    NSString *path = [Utilities generateFilePathWithFilename:filename projectId:self.projectId];
...
- (void)recordingStatusDidUpdate {
    ...
    [self.recordingDelegate audioManager:self didUpdateRecordProgress:progress currentTime:self.currentRecordingTime];
