...
FFPopupLayout FFPopupLayoutMake(FFPopupHorizontalLayout horizontal, FFPopupVerticalLayout vertical, CGFloat offset) {
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.closeButton.hidden = !self.shouldShowClose;
...
- (void)setShouldShowClose:(BOOL)shouldShowClose {
    _shouldShowClose = shouldShowClose;
    self.closeButton.hidden = !shouldShowClose;
}
...
- (void)showWithParameters:(NSDictionary *)parameters {
    ...
    if (!_isBeingShown && !_isShowing && !_isBeingDismissed) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //[NSLayoutConstraint activateConstraints:@[[NSLayoutConstraint constraintWithItem:strongSelf.closeButton.topAnchor attribute:<#(NSLayoutAttribute)#> relatedBy:<#(NSLayoutRelation)#> toItem:<#(nullable id)#> attribute:<#(NSLayoutAttribute)#> multiplier:<#(CGFloat)#> constant:<#(CGFloat)#>]]];
            //[NSLayoutConstraint activateConstraints:@[[strongSelf.closeButton.topAnchor constraintEqualToAnchor:strongSelf.topAnchor constant:strongSelf.safeAreaInsets.top], [strongSelf.closeButton.trailingAnchor constraintEqualToAnchor:strongSelf.trailingAnchor constant:4.0], [strongSelf.closeButton.widthAnchor constraintEqualToConstant:64.0], [strongSelf.closeButton.heightAnchor constraintEqualToConstant:64.0]]];
            ...
            if (centerValue) {
                ...
                switch (layout.vertical) {
                    ...
                    case FFPopupVerticalLayout_Bottom:
                        finalContainerFrame.origin.y = CGRectGetHeight(self.bounds) - CGRectGetHeight(containerFrame) - offset;
...
- (UIView *)closeButton {
    if (!_closeButton) {
        ...
        _closeButton.autoresizesSubviews = NO;
        _closeButton.userInteractionEnabled = YES;
        _closeButton.backgroundColor = UIColor.clearColor;
        [_closeButton addTarget:self action:@selector(dismiss) forControlEvents:UIControlEventTouchUpInside];
        _closeButton.frame = CGRectMake(self.frame.size.width - 68, self.safeAreaInsets.top, 64, 64);
    }
    return _closeButton;
}
