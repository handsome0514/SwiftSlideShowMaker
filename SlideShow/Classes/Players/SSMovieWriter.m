...
#import "SSMovieWriter.h"

@interface SSMovieWriter ()
{
...
- (void)initializeMovieWithOutputSettings:(NSDictionary *)outputSettings;
{
    ...
    assetWriter.movieFragmentInterval = kCMTimeInvalid; // CMTimeMakeWithSeconds(1.0, 1000);
...
//        settings[AVVideoCompressionPropertiesKey] = @{AVVideoAverageBitRateKey:@(25000000),
//                                                      AVVideoMaxKeyFrameIntervalKey:@(1),
...
-(void)newFramesReadyAtTimes:(NSArray<NSValue*>*)frameTimes atIndex:(NSInteger)textureIndex {
    ...
    CMTime frameTime = frameTimes.firstObject.CMTimeValue;
...
//    runAsynchronouslyOnContextQueue(_movieWriterContext, ^{
        ...
        void(^write)() = ^() {
            CMTime time = frameTime;
            for (NSValue* value in frameTimes) {
                CMTime currentFrameTime = value.CMTimeValue;
                ...
                {
                    NSLog(@"2: Had to drop a video frame: %@", CFBridgingRelease(CMTimeCopyDescription(kCFAllocatorDefault, currentFrameTime)));
                }
                else if(self.assetWriter.status == AVAssetWriterStatusWriting)
                {
                    if (![assetWriterPixelBufferInput appendPixelBuffer:pixel_buffer withPresentationTime:currentFrameTime])
                        NSLog(@"Problem appending pixel buffer at time: %@", CFBridgingRelease(CMTimeCopyDescription(kCFAllocatorDefault, currentFrameTime)));
                }
                else
                {
